...
$(function () {
	...
	$(document).on('startDisplay', function(e, communityId, clusterId, unitId, communityOutline) {
		 ...
		 * @param {Date} periodFrom 融通情報の検索日付 (開始)
		 * @param {Date} periodTo 融通情報の検索日付 (終了)
		...
		function doDisplayDeals(periodFrom, periodTo, deals) {
			...
			for (const aDeal of deals) {
				if (aDeal.cumulateAmountWh) {
					...
					if (aDeal.dischargeUnitId == unitId) {
						dischargeSum += aDeal.cumulateAmountWh;
					}
					if (aDeal.chargeUnitId == unitId) {
						chargeSum += aDeal.cumulateAmountWh;
					}
				}
			...
			const theUnit = communityOutline.communities[communityId].clusters[clusterId].units[unitId];
			const un = theUnit.name;
			const dchg = dischargeSum || 0;
			const chg = chargeSum || 0;
		...
		function showDealsDialog(periodFrom, periodTo, deals) {
			...
			for (const aDeal of deals) {
				...
				for (const key in aDeal) {
					if (key.endsWith('UnitId')) {
						if (aDeal[key] == theUnit.id) aDeal[key] = theUnit.name;
					}
				}
