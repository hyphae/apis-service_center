...
$(function () {
	 ...
	 * 設定ダイアログを表示し設定する
	 * 
	 * コミュニティ/クラスタ選択, 画面選択, およびログアウト機能を提供する.
			...
			buttons: [
				{
					...
					click: function() {
						...
						if (selectedCommunityId && selectedClusterId) {
							// グローバル変数を更新する
	...
	const loadCommunityOutline = function(onSuccess) {
		...
		client.community.outline(
			function(json, textStatus) {
				...
				// 現在選択中のコミュニティ/クラスタが正しい値・組み合わせである ?
				if (communityOutline.communities[communityId] && communityOutline.communities[communityId].clusters[clusterId]) {
					// ok
				} else {
					// 現在のコミュニティ/クラスタの選択状態を解除する
					communityId = null;
					clusterId = null;
				}
				onSuccess();
	...
	const settingUpdated = function() {
		...
		// 表示処理を起動する
		$(document).trigger('startDisplay', [communityId, clusterId, communityOutline]);
	}
